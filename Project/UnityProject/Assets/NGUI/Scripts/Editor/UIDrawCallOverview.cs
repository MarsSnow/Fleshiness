...
public class UIDrawCallOverview : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        for (int i = 0; i < UIDrawCall.activeList.size; ++i)
        {
            ...
            string name = "<b>" + key + " of " + UIDrawCall.activeList.size + " (" + dc.panel.name + ")</b>";
            ...
            if (GUILayout.Button("Select panel: <b>" + dc.panel.name + "</b>", "dragtab", new GUILayoutOption[] { GUILayout.ExpandWidth(false), GUILayout.MinWidth(250f)}))
            {
            ...
            for (int b = 0; b < dc.panel.widgets.Count; ++b)
            {
                UIWidget w = dc.panel.widgets[b];
                int depth = w.depth;
